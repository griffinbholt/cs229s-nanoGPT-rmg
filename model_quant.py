...
class CausalSelfAttention(nn.Module):
    ...
    def decode(self, x):
        ...
        att = att.masked_fill(self.bias[:,:,self.seq_pos,:newSeqPos] == 0, float('-inf'))
        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)
        y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head outputs side by side
...
class GPT(nn.Module):
    ...
    @classmethod
    def from_quantized(cls, model_type, override_args=None):
        assert model_type in {'gpt2-quantized', 'gpt2-quantized-medium', 'gpt2-quantized-large'}
        override_args = override_args or {} # default to empty dict
        # only dropout can be overridden see more notes below
        assert all(k == 'dropout' for k in override_args)
        print("loading quantized weights from pretrained gpt: %s" % model_type)

        # n_layer, n_head and n_embd are determined from model_type
        config_args = {
            'gpt2-quantized':         dict(n_layer=12, n_head=12, n_embd=768),  # 124M params
            'gpt2-quantized-medium':  dict(n_layer=24, n_head=16, n_embd=1024), # 350M params
    ...
    def generate_speculative(self, idx, max_new_tokens, draft_model, temperature=1.0, top_k=None, num_speculative=4):
        ...
        while idx.size(1) < idx_length_original+max_new_tokens:
            ...
            for i in range(idx_cond.size(1), idx_speculative.size(1)+1):
                ...
                # optionally crop the logits to only the top k options
                if top_k == 1:
                    next_idx = torch.argmax(cur_logits, dim=-1).reshape((-1,1))
                else:
                    #if top_k is not None:
                ...
                if i < idx_speculative.size(1) and next_idx != idx_speculative[:, i]:
                    break
