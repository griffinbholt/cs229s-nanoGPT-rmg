...
class LinearLoRA(nn.Module):
    def __init__(self, in_dim, out_dim, rank=8, lora_alpha=32, lora_dropout=0):
        super().__init__()
        self.rank = rank
...
def add_lora_layers(model, rank=8, lora_alpha=16, lora_dropout=0.0):
    ...
    for name, module in model.named_modules():
        ...
        # if (name.endswith("c_proj") or 
        #     name.endswith("c_attn") or 
        #     name.endswith("c_fc") or 
        #     name == ("lm_head") # gross
        # ):
            to_change.append((name,module))
    
    # print(to_change)
    for name, module in to_change:
